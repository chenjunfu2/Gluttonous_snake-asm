...
;---------------------程序开始---------------------;
	...
	;初始化完毕，开始游戏循环
	;winxp没有int15h的83h功能号，改为使用PIT可编程定时器通道2
	mov al,10110000b;10->通道2 11->先读写低字节后读写高字节 000->模式0（时间到停止，等待下次设置） 0->二进制数据，否则bcd码
	out 43h,al;al控制字写入0x43 模式/命令寄存器（只写，读取被忽略）
	...
	game_loop:
		...
		pit_test:
			in al,61h;从61h端口读出当前通道2的状态
			test al,00100000b;判断bit5是否为1，为0继续测试
		jz pit_test


		;设置计数值1193182hz / 1000hz=1ms
		mov al,0h
		out 42h,al;0x42 通道2数据端口（读/写）低位计数值
		mov al,0h
		out 42h,al;0x42 通道2数据端口（读/写）高位计数值
		;读写完毕后自动开始计数
